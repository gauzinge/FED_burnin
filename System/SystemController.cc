...
void SystemController::parseHWxml( const std::string& pFilename, std::ostream& os )
{
	...
	for ( pugi::xml_node cBeBoardNode = cShelveNode.child( "PixFED" ); cBeBoardNode; cBeBoardNode = cBeBoardNode.next_sibling() )
	{
	...
	fPixFEDFWInterface = new PixFEDFWInterface( /*fBeBoardFWMap*/ );
	os << "\n";
	os << "\n";
	for ( i = 0; i < 80; i++ )
		os << "*";
